...
document.addEventListener("click", event => {
    ...
    window.ExplorerApp.prepareRequest();
...
// Read-only bridge: the shared Explorer reads the original Jaguar itinerary.
window.JaguarExpedition = Object.freeze({
    isComplete: () => pousadasEscolhidas().length === MAX_POUSADAS,
    stops: () => roteiro.map(item => ({ nome: item.nome, noites: item.dias || DIAS_POR_PARADA })),
    openItinerary: () => { atualizarRoteiro(); roteiroPanel.classList.add("show"); }
});
